...
## 关于server中基类entity的整合

现阶段actor和itementity是割裂的，他们传送是用各自的协议，这样显得entity同步有点没用了。

其实可以模仿mmo课程中的那种做法，虽然之前觉得不太优雅，但是找不到更好的方案了



## 关于server中netService中的bug

部署与服务器时，解码器中，居然不能捕获socket为空的异常来解决，颇为奇怪。
