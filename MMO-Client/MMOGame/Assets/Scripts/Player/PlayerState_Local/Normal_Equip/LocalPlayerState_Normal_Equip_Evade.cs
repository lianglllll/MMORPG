...
{
    public class LocalPlayerState_Normal_Equip_Evade : LocalPlayerState
    {
        private Coroutine rollCorotine;
        private bool isRotate = false;
        private string curAnimationName = "Normal_Equip_Evade_Back";
        ...
        public override void Enter()
        {
            ...
            else
            {
                ...
                curAnimationName = "Normal_Equip_Evade_Back";
                player.PlayAnimation(curAnimationName);
        ...
        public override void Update()
        {
            ...
            if (CheckAnimatorStateName(curAnimationName, out var time) && time >= 0.9f)
            {
                float h = GameInputManager.Instance.Movement.x;
                float v = GameInputManager.Instance.Movement.y;
                if (h != 0 || v != 0)
                {
                    ...
                    var state = (LocalPlayerState_Normal_Equip_Motion)player.stateMachine.CurState;
                    state.SetRun();
                }
                else
                {
        ...
        private IEnumerator DoRotate(Quaternion targetRotation)
        {
            ...
            curAnimationName = "Normal_Equip_Evade_Front";
            player.PlayAnimation(curAnimationName);
            player.Model.SetRootMotionAction(OnRootMotion);
        }
        private IEnumerator DoRotate2(NetAcotrEvadeStatePayload payload, Quaternion targetRotation)
        {
            ...
            {
                curAnimationName = "Normal_Equip_Evade_Left";
            }
            else if (payload == NetAcotrEvadeStatePayload.NetActorEvadeStatePayloadRight)
            {
                curAnimationName = "Normal_Equip_Evade_Right";
            }
            player.PlayAnimation(curAnimationName);
            player.Model.SetRootMotionAction(OnRootMotion);
        }
