...
public class SecurityService : SingletonNonMono<SecurityService>
{
    ...
    public bool SendExchangePublicKeyRequest()
    {
        ...
        req.ClientPublicKey = NetManager.Instance.m_curNetClient.EncryptionManager.GetLocalRsaPublicKey(); 
    ...
    {
        NetManager.Instance.m_curNetClient.EncryptionManager.SetRemoteRsaPublicKey(message.ServerPublilcKey);
        SendExchangeCommunicationSecretKeyRequest();
    }

    public bool SendExchangeCommunicationSecretKeyRequest()
    {
        var kv = NetManager.Instance.m_curNetClient.EncryptionManager.GenerateAesKeyAndIv();
        NetManager.Instance.m_curNetClient.EncryptionManager.SetAesKeyAndIv(kv.key, kv.iv);
        ExchangeCommunicationSecretKeyRequest req = new();
        // 用对方的rsa公钥加密两个key
        req.Key1 = NetManager.Instance.m_curNetClient.EncryptionManager.RsaEncrypt(kv.key);
        req.Key2 = NetManager.Instance.m_curNetClient.EncryptionManager.RsaEncrypt(kv.iv);
        NetManager.Instance.m_curNetClient.Send(req);
        return true;
    }
