...
namespace GameGateMgrServer.Handle
{
    public class GameGateMgrHandler : Singleton<GameGateMgrHandler>
    {
        ...
        private void _HandleClusterEventResponse(Connection sender, ClusterEventResponse message)
        {
            ...
            {
                Log.Information("A new Game server has joined the cluster, {0}", message.ClusterEventNode.ServerInfoNode);
                GGMMonitor.Instance.AddGameServerInfo(message.ClusterEventNode.ServerInfoNode);
            }
            ...
            {
                Log.Error("Unknown ccEvent, type = {0}", message.ClusterEventNode.EventType.ToString());
            }
        }
